...
## 13. Facturation & devis — facturation électronique obligatoire (France)

**Règle : toute app qui émet des factures ou des devis B2B en France doit produire des factures ÉLECTRONIQUES conformes.** Ne jamais concevoir un simple PDF « imprimé » comme facture : il doit être structuré et transitable via une plateforme agréée.

### Calendrier de l'obligation (réforme confirmée)
- **1er septembre 2026** : **réception** de factures électroniques obligatoire pour **toutes** les entreprises assujetties à la TVA ; **émission** obligatoire pour les grandes entreprises et ETI.
- **1er septembre 2027** : **émission + e-reporting** obligatoires pour PME, TPE et micro-entreprises.
- Concerne les opérations **B2B domestiques** (assujettis TVA établis en France). Le **B2C et l'international** relèvent de l'**e-reporting** (transmission des données de transaction/paiement à l'administration).

### Comment faire (à implémenter dans l'app)
1. **Format structuré obligatoire** : produire la facture dans un des formats socle — **Factur-X** (recommandé PME/TPE : PDF/A-3 lisible **avec XML CII intégré**), **UBL** ou **CII**. Générer le XML, pas seulement le PDF.
2. **Transiter par une Plateforme Agréée (PA / ex-PDP)** : depuis l'abandon du portail public gratuit (PPF) le 15/10/2024, l'émission/réception passe **obligatoirement par une plateforme privée immatriculée** par l'État (100+ disponibles). L'app doit **s'interconnecter à une PA via API** (ne pas réinventer le transport). Le PPF ne garde qu'un rôle d'**annuaire central** + concentrateur de données fiscales.
3. **Mentions obligatoires enrichies (dès le 01/09/2026)** : en plus des mentions légales classiques (identité, SIREN émetteur, n° de facture, date, désignation, quantités, prix HT/TTC, taux et montant de TVA, conditions de paiement/pénalités) → ajouter **le SIREN de l'acheteur**, **la catégorie de l'opération** (livraison de biens / prestation de services / mixte) et **l'option TVA sur les débits** le cas échéant.
4. **E-reporting** : transmettre les **données de transaction** (B2C, international) et de **paiement** à l'administration via la PA.
5. **Devis** : un devis n'est pas une facture, mais le concevoir dès le départ pour qu'il puisse **se convertir en facture électronique conforme** (mêmes données structurées, mêmes identifiants).
6. **Conservation** : archiver les factures (électroniques) avec valeur probante pendant la durée légale (**10 ans** en comptabilité ; 6 ans fiscal).

### Sanctions (à éviter absolument)
Défaut de PA : **500 €** (puis 1 000 € tous les 3 mois) ; **15 € par facture** non conforme (plafond 15 000 €/an) ; **250 €** par manquement d'e-reporting.

> **Consigne d'implémentation** : pour toute app facturant en France, intégrer dès la conception un module de facturation qui génère du **Factur-X/UBL/CII** et s'appuie sur une **Plateforme Agréée via API** — pas un générateur de PDF maison. Vérifier la liste des PA à jour sur impots.gouv.fr avant de choisir le connecteur.

---

## 14. Conformité juridique — être « ultra clean » (France / UE)

**Principe : rien ne doit être attaquable.** Avant de publier une app, valider la conformité selon 3 questions : (1) qui est l'éditeur ? (2) l'app fait-elle payer ? (3) y a-t-il du contenu utilisateur, de l'IA, un secteur réglementé ? Adapter les obligations à **la cible** (grand public, mineurs, pros, données sensibles). *Ce guide n'est pas un avis juridique individualisé — pour les décisions engageantes (agrément ACPR, qualification dispositif médical, marketplace), faire valider par un conseil spécialisé.*

### 14.1 RGPD / données personnelles (socle de toute app avec compte)
- **Base légale par finalité** (art. 6), choisie avant le traitement (contrat / consentement / intérêt légitime documenté). Pas de cumul.
- **Consentement cookies/traceurs** (art. 82 LIL) : bouton **« Tout refuser » au même niveau que « Tout accepter »**, aucun traceur non essentiel avant consentement, preuve conservée. Suivre la **recommandation CNIL applications mobiles (2025)**.
- **Registre des traitements** (art. 30) — en pratique obligatoire même en solo.
- **Politique de confidentialité dédiée** (art. 12-14), distincte des CGU, accessible **dans l'app** et au moment de la collecte.
- **DPO** (art. 37) si suivi systématique à grande échelle ou données sensibles à grande échelle. **AIPD/DPIA** (art. 35) si risque élevé (≥ 2 des 9 critères EDPB, ou liste CNIL) — **systématique** pour données de santé/mineurs.
- **Minimisation + durées de conservation** définies par catégorie, purge automatisée (art. 5).
- **Droits des personnes** (art. 15-22) : accès, rectification, **effacement**, portabilité/**export**, opposition — réponse **sous 1 mois**. Prévoir techniquement **suppression de compte** et export.
- **Privacy by design & by default** (art. 25) : options intrusives (géoloc, profil public) **OFF par défaut**.
- **Sécurité** (art. 32) : **chiffrement TLS + au repos**, MFA, moindre privilège, journalisation, sauvegardes ; **contrat art. 28 avec chaque sous-traitant** ; **transferts hors UE** encadrés (adéquation / EU-US DPF / CCT + TIA).
- **Violation de données** : notification CNIL **sous 72 h** si risque + registre interne des violations. Procédure d'incident prête d'avance.
- **Mineurs** : **âge du consentement numérique = 15 ans** en France ; < 15 ans → **consentement parental**, réglages protecteurs par défaut (8 recommandations CNIL).
- **Santé** : consentement explicite (art. 9) + **hébergeur certifié HDS** ; **bancaire** : ne jamais stocker le CVV, PSP **PCI-DSS**.

### 14.2 Documents obligatoires
- **Mentions légales** (LCEN art. 6) : identité éditeur, directeur de publication, **hébergeur**. Sanction : 1 an / 75 000 €.
- **CGU** (acceptation horodatée), **CGV** si vente, **politique de confidentialité + cookies**, **infos précontractuelles** dans le tunnel d'achat.

### 14.3 Droit de la consommation (si B2C)
- **Rétractation 14 jours** (L221-18) ; **contenu numérique** : valable uniquement avec **case explicite de renonciation** + exécution immédiate (L221-28).
- **Médiateur de la consommation** : **adhésion obligatoire dès la 1ère vente B2C** + coordonnées publiées (souvent oublié).
- **Garanties légales** (conformité biens 2 ans + **contenus/services numériques** incl. mises à jour + vices cachés).
- **Prix TTC** + total avant validation ; **avis clients** transparents.
- **Résiliation « en 3 clics »** (L215-1-1) pour tout abonnement.
- **Pratiques déloyales/dark patterns** (L121-1) : interdits — cf. section 8. Sanction jusqu'à 2 ans / 300 000 € ou 10 % du CA.

### 14.4 E-commerce / TVA / paiement
- **TVA** : services numériques B2C taxés **au pays du client** ; **seuil unique 10 000 € HT/an UE** → au-delà, **guichet OSS** (impots.gouv.fr). Franchise en base France (services 37 500 / 41 250 €) ne dispense pas de l'OSS.
- **Facturation électronique** : cf. section 13.
- **Paiement DSP2** : **authentification forte (SCA / 3-D Secure 2)** côté PSP ; **Stripe Checkout/Elements** (périmètre PCI **SAQ A**) ; jamais de CVV stocké. Numérique consommé in-app → **IAP Apple / Play Billing** (sauf options DMA UE).

### 14.5 Règlements européens transverses
- **DSA** (applicable 2024) : dès qu'il y a du **contenu utilisateur** → point de contact, CGU de modération, **mécanisme de signalement + motivation**, **zéro dark pattern (art. 25)**, pub identifiable, pas de ciblage des mineurs. Autorité FR : **ARCOM**. (Micro/petites entreprises dispensées de certaines obligations plateforme.)
- **AI Act** : si l'app utilise de l'IA (chatbot, génération de contenu) → **transparence art. 50** (signaler l'IA, **marquer les contenus générés**, divulguer deepfakes), applicable **2 août 2026**. Vérifier l'absence de pratique interdite (art. 5).
- **European Accessibility Act** : **obligatoire depuis le 28 juin 2025** pour e-commerce, banque, transports → **EN 301 549 / WCAG 2.1 AA / RGAA** + **déclaration d'accessibilité**. Exemption microentreprise de services (< 10 sal. et ≤ 2 M€). Cf. section 5.

### 14.6 App stores
- **Apple** : App Privacy Details (+ SDK tiers), politique de confidentialité, **suppression de compte in-app obligatoire**, IAP, classification d'âge, Sign in with Apple si login social, **ATT** pour le suivi cross-app.
- **Google Play** : **Data safety**, politique de confidentialité (fiche + in-app), **suppression compte/données in-app ET web**, Play Billing, content rating IARC, Families policy si audience enfants.

### 14.7 Secteurs réglementés (régime + autorité)
Santé/e-santé (HDS, MDR/dispositif médical → **CNIL, ANS, ANSM**) · Finance/paiement (agrément → **ACPR**) · Jeux d'argent (**ANJ**, casino en ligne interdit) · Crypto (MiCA, agrément **CASP → AMF** ; fin période transitoire PSAN au 1er juillet 2026) · Assurance (DDA, **ORIAS + ACPR**).

### 14.8 Propriété intellectuelle & marque
- **Marque INPI** : recherche d'antériorité + dépôt (app → **classe 9** logiciels + **classe 42** SaaS ; ~190 €/classe, 10 ans). Marque UE via EUIPO (850 €).
- **Nom de domaine** (.fr via AFNIC) réservé de façon coordonnée avec la marque.
- **Droit d'auteur code/design** : automatique ; **dépôt e-Soleau INPI** pour la preuve de date ; **cession des droits** des prestataires sécurisée par contrat.
- **Open-source** : respecter les licences (attention **AGPL** en SaaS) ; tenir un **SBOM** + écran « Licences » dans l'app.

### 14.9 Checklist de conformité par type d'app
- **App vitrine** : mentions légales, politique de confidentialité + cookies conformes, analytics exempté ou consenti, HTTPS, accessibilité si concernée, marque + domaine.
- **App avec compte** : + base légale/finalité, registre, information art. 12-14, **suppression de compte**, exercice des droits, durées + purge, contrats sous-traitants, transferts encadrés, procédure violation 72 h, DPO/AIPD si requis, DSA si UGC, AI Act si IA, App Privacy/Data safety.
- **App e-commerce/paiement** : + CGV, infos précontractuelles + rétractation, médiateur conso, garanties, prix TTC, résiliation 3 clics, TVA/OSS, **facturation électronique (PA)**, PCI-DSS/SCA, IAP/Play Billing.
- **App données sensibles** (santé/finance/mineurs/crypto) : + **AIPD systématique**, dérogation art. 9 + **HDS** (santé), consentement parental < 15 ans, agrément sectoriel (ACPR/ANJ/AMF/ORIAS), sécurité renforcée.

### 14.10 Maintenance juridique (à tenir à jour)
Le droit du numérique évolue vite. **Revoir cette section au moins 2×/an** et à chaque lancement, en vérifiant les sources officielles (CNIL, economie.gouv.fr, impots.gouv.fr, Commission européenne, ARCOM). Points datés à surveiller : e-invoicing (sept. 2026/2027), AI Act art. 50 (août 2026, ajusté par l'« AI Omnibus »), fin transition crypto PSAN→CASP (1er juillet 2026), évolutions cookies CNIL, futur règlement ePrivacy, **Digital Fairness Act** (proposition attendue 2026). Voir le dossier de suivi institutionnel dans `Colophon/dossiers-conformite/`.
...
**Juridique & facturation (France/UE)** : economie.gouv.fr (facturation électronique) · impots.gouv.fr (plateformes agréées, OSS) · service-public.fr · CNIL.fr (RGPD, cookies, recommandation apps mobiles 2025, mineurs) · Légifrance (LCEN, Code conso, LIL) · Commission européenne (DSA, AI Act, European Accessibility Act) · ARCOM (DSA) · INPI/EUIPO (marques). Formats facture : Factur-X / UBL / CII.
