-- Lieux de lecture (librairies, festivals, cafés philo, cercles de lecture)
-- Carte interactive Colophon. Import du CSV : lieux_lecture_france.csv
...
-- Recommandations lecteurs (coups de cœur) : pointe vers un place + un user.
create table if not exists public.place_recommendations (
  id          uuid primary key default gen_random_uuid(),
  place_id    text not null references public.places(id) on delete cascade,
  user_id     uuid not null references auth.users(id) on delete cascade,
  rating      numeric(2,1),               -- demi-étoiles, comme items.rating
  comment     text,
  created_at  timestamptz not null default now(),
  unique (place_id, user_id)
);
alter table public.place_recommendations enable row level security;
create policy "recos readable by all"
  on public.place_recommendations for select using (true);
create policy "users manage own recos"
  on public.place_recommendations for all
